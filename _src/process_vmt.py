...
PROCESSED_DIR = REPO_ROOT / "_data" / "processed" / "vmt"
...
def resolve_vmt_columns(field_names: list[str], path: Path) -> dict:
    ...
    for output_col, candidate_cols in VMT_COLUMN_ALIASES.items():
        source_col = next(
            (candidate_col for candidate_col in candidate_cols if candidate_col in field_name_set),
            None,
        )
...
def read_vmt_metrics_for_file(path: Path) -> pd.DataFrame:
    ...
    records = [
        {"CO_TAZID": taz_id, **values}
        for taz_id, values in sorted(totals.items())
    ]
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Build and publish VMT data artifacts for the Vite web app."
    )
    parser.add_argument(
        "--processed-only",
        action="store_true",
        help=(
            "Build canonical outputs in _data/processed/vmt without copying "
            "them to _site/public/data/vmt."
        ),
    )
    parser.add_argument(
        "--publish-only",
